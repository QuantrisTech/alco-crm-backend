...
const createSeo = async (req, res) => {
  try {
    const { pageSlug, pageLabel } = req.body;

    if (!pageSlug || !pageLabel) {
      return res.status(400).json({ success: false, message: "pageSlug and pageLabel are required" });
    }

    const exists = await SeoMeta.findOne({ pageSlug: pageSlug.toLowerCase() });
    if (exists) {
      return res.status(409).json({ success: false, message: "SEO entry already exists. Use update." });
    }

    const seo = await SeoMeta.create({ ...req.body, lastUpdatedBy: req.user._id });
    return res.status(201).json({ success: true, message: "SEO created", data: seo });
  } catch (err) {
    return res.status(500).json({ success: false, message: "Server error" });
  }
};

// ADMIN — Update by slug
// PUT /api/v1/seo/:pageSlug
const updateSeo = async (req, res) => {
  try {
    const seo = await SeoMeta.findOne({ pageSlug: req.params.pageSlug.toLowerCase() });
    if (!seo) return res.status(404).json({ success: false, message: "SEO not found" });

    const allowed = ["pageLabel", "title", "description", "canonical", "robots", "openGraph", "twitter", "keywords", "structuredData", "isActive"];
    allowed.forEach((f) => { if (req.body[f] !== undefined) seo[f] = req.body[f]; });
    seo.lastUpdatedBy = req.user._id;

    await seo.save();
    return res.status(200).json({ success: true, message: "SEO updated", data: seo });
  } catch (err) {
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
...
// DELETE /api/v1/seo/:pageSlug
const deleteSeo = async (req, res) => {
  try {
    const seo = await SeoMeta.findOneAndDelete({ pageSlug: req.params.pageSlug.toLowerCase() });
    if (!seo) return res.status(404).json({ success: false, message: "SEO not found" });

    return res.status(200).json({ success: true, message: "SEO deleted" });
  } catch (err) {
    return res.status(500).json({ success: false, message: "Server error" });
  }
};

// ADMIN — Upsert (create or update) — MAIN ENDPOINT
// PATCH /api/v1/seo/upsert
const upsertSeo = async (req, res) => {
  try {
    const { pageSlug, pageLabel, ...rest } = req.body;

    if (!pageSlug) return res.status(400).json({ success: false, message: "pageSlug required" });

    const seo = await SeoMeta.findOneAndUpdate(
      { pageSlug: pageSlug.toLowerCase() },
      { pageSlug: pageSlug.toLowerCase(), pageLabel: pageLabel || pageSlug, ...rest, lastUpdatedBy: req.user._id },
      { new: true, upsert: true, runValidators: true, setDefaultsOnInsert: true }
    );

    return res.status(200).json({ success: true, message: "SEO saved", data: seo });
  } catch (err) {
    return res.status(500).json({ success: false, message: "Server error" });
  }
};

module.exports = { getSeoBySlug, getAllSeo, createSeo, updateSeo, deleteSeo, upsertSeo };